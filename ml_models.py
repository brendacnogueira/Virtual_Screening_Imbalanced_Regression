...
# %%
def sequence_to_aac(seq):
    try:
        desc = GlobalDescriptor(seq)
        desc.calculate_aac()
        return desc.descriptor[0]
    except:
        return np.zeros(20)
...
for target in regression_tids:
    ...
    # Constructing Dataset
        ...
        for sel_metric in selection_metric:
            ...
            # Save ML models
            result_path = create_directory(f'./regression_results/')
            for trial, (train_idx, test_idx) in enumerate(data_splitter.split(dataset.features, dataset.target)):
                ...
                ph={"method":"extremes","npts": 3,"control_pts":[0,0,0,11,1,0]}
            ...
            predictions_test_df.to_csv(os.path.join(result_path, f'{target}_{model}_{sel_metric}.csv'))
