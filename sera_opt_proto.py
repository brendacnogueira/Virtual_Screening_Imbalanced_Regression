...
def sert(y, y_hat, phi, t, include_equals=True,device="cpu"):
    ...
    #0's otherwise
    t  = torch.as_tensor(t, dtype=y.dtype, device=device)
    if include_equals:
...
def sera_pt(y, y_hat, phi, include_equals=True, resolution=1000,device="cpu"):
    ...
    ts = torch.linspace(0, 1, resolution, device=device)
    sert_points = sert_v(ts).to(device)
...
class SeraCriterion(nn.Module):
    ...
    def forward(self, y_hat,y):
        ...
        phi_values   = phi_values.to(self.device)
